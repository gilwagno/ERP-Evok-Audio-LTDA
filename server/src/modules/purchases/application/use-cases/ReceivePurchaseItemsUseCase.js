 ...
 * continua 100% delegada a `InventoryService.receive` (mesma
 * `server/src/services/inventoryService.js` usada pelo controller legado),
...
class ReceivePurchaseItemsUseCase extends UseCase {
  ...
  async execute({ id, items, userId, transaction }) {
    ...
    for (const received of items) {
      ...
      await InventoryService.receive(item.product_id, qty, transaction, {
        user_id: userId,

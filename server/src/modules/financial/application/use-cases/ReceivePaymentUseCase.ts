...
/**
 * Registra o recebimento (total ou parcial, via ajuste de `amount`) de uma
 * conta a receber, cobrindo o fluxo do endpoint
 * `PUT /api/finance/receivable/:id/pay`.
 *
 * Reproduz exatamente as validações do controller anterior
 * `server/src/controllers/financeController.ts#receivePayment`.
 */
class ReceivePaymentUseCase extends UseCase {
   ...
   * @param {number|string} [input.amount] - Valor recebido; se informado, deve ser > 0 e <= saldo da conta.
   * @returns {Promise<{ account: Object, previousStatus: string }>}
   * @throws {NotFoundError} Se a conta a receber não existir.
  ...
  async execute({ id, payment_date, payment_method, amount }) {
    const account = await this.financialRepository.findReceivableById(id);

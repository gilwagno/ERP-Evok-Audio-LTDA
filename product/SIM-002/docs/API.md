...
## Identidade e papéis (APR-2026-008)
...
**Matriz de autorização aprovada (APR-2026-008):**
...
| `payments.cancelPayment` | escrita | `analyst`, `manager` — **lacuna normativa**: APR-2026-007 definiu a semântica do cancelamento, não o papel exigido |

Em todas as operações vale BR-SEC-001: o alcance é limitado à empresa do
usuário resolvida no banco.

O objeto `approver` de `approvals.approveSupplier` ainda é autodeclarado —
`approvalService` está fora do escopo de APR-2026-008 e permanece como lacuna
conhecida (ver pacote de evidência da WAVE-D).
...
- **Saída (recusa do gateway — APR-2026-009):** pagamento com
  ...
  pagamento recusado **não** conta como enviado e pode ser reenviado; a trilha
  de tentativas preserva cada recusa.
- **Erros:** `Usuário inválido`, `Usuário não autenticado`,
...
- **Papel exigido:** usuário autenticado da empresa proprietária (`analyst` ou
  `manager`). APR-2026-007 não arbitrou papel específico para o cancelamento —
  lacuna normativa registrada.
- **Entrada:** `paymentId`, `user` (apenas `user.id`).
...
- **Referências:** BR-SEC-001, BR-PAY-002, APR-2026-007.
